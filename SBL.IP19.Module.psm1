...
function Invoke-SBLWFP {
    ...
    $Result = Invoke-WebRequest -Uri $URI -ContentType application/json -Method POST -Credential $Credential -Verbose:$Verbose -DisableKeepAlive 
...
## Change the AI Auth Token (using SADMIN user)  ##
function Set-SBLAIAuthToken {
    param ([string[]]$Servers = $global:SBLAIServers,
            [string]$Path     = $SBLAppIntPath,
            [string]$Username = "SADMIN",
            [string]$NewPassword,
            [Management.Automation.PSCredential]$Credential,
            [switch]$Verbose)

    $sEncodedString = [Convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes(("{0}:{1}" -f $Username, $NewPassword)))
    $Command1       = ("return {0}\jre\bin\java -jar {1}\applicationcontainer\webapps\siebel\WEB-INF\lib\EncryptString.jar {2}" -f $Path,$Path,$sEncodedString)

    foreach ($Server in $Servers) {
        $EncodedString = Invoke-Command -ComputerName $Server -ScriptBlock ([Scriptblock]::Create($Command1)) -Credential $Credential -Verbose:$Verbose

        $Command2 = "(Get-Content {0}\applicationcontainer\webapps\applicationinterface.properties) -Replace ""(AuthToken=)(.)*"", ""AuthToken={1}"" | Set-Content {2}\applicationcontainer\webapps\applicationinterface.properties" -f $Path, $EncodedString, $Path

        Invoke-Command -ComputerName $Server -ScriptBlock ([Scriptblock]::Create($Command2)) -Credential $Credential -Verbose:$Verbose
    }
}
function Stop-SBLAIServer {
    param ([string[]]$Servers = $global:SBLAIServers,
           [Management.Automation.PSCredential]$Credential,
           [switch]$Verbose)

    Write-Verbose -Message ("Stoping Siebel AI Service on '{0}'" -f $Servers) -Verbose:$Verbose
    Invoke-Command -ComputerName $Servers -ScriptBlock {  stop-service -InputObject $(get-Service | where {$_.Name -like '*_ai'}) } -Credential $Credential -Verbose:$Verbose
}

function Start-SBLAIServer {
    param ([string[]]$Servers = $global:SBLAIServers,
            [Management.Automation.PSCredential]$Credential,
            [switch]$Verbose)

    Write-Verbose -Message ("Start Siebel AI Service on '{0}'" -f $Servers) -Verbose:$Verbose
    Invoke-Command -ComputerName $Servers -ScriptBlock { start-service -InputObject $(get-Service | where {$_.Name -like '*_ai'}) } -Credential $Credential -Verbose:$Verbose
}

function Get-SBLAIServer {
    param ([string[]]$Servers = $global:SBLAIServers,
           [Management.Automation.PSCredential]$Credential)

    Invoke-Command -ComputerName $Servers -ScriptBlock { get-Service | where {$_.Name -like '*_ai'} } -Credential $Credential
}

## Siebel Gateway Functions ##
...
function Set-SBLGtwyEnvParam {
  ...
  $lGtwyCmd   = '{0}\BIN\srvrmgr.exe -g {1}:{2} -e siebel -u sadmin -p "{3}" -c "change ent param {4}=""{5}"""'

  Write-Verbose -Message "Gateway update enterpize paramiter $Name." -Verbose:$Verbose
  Invoke-Command -ComputerName $GatewayServer -ScriptBlock ([Scriptblock]::Create(($lGtwyCmd -f $Path,$GatewayServer,$global:$SBLGtwyPort,$Password,$Name,$Value))) -Credential $Credential -Verbose:$Verbose
}
...
function Set-SBLGtwySubSysParam {
    ...
    $lGtwyCmd01   = '{0}\BIN\srvrmgr.exe -g {1}:{2} -e siebel -u sadmin -p "{3}" -c "change param {4}=""{5}"" for named subsystem {6}"'

    foreach ($SybSystem in $SybSystems) {
        Write-Verbose -Message ("Gateway update SubSystem paramiter '{0}' for '{1}'." -f $Name,$SybSystem)
        Invoke-Command -ComputerName $ComputerName -ScriptBlock ([Scriptblock]::Create(($lGtwyCmd01 -f $Path,$GatewayServer,$global:$SBLGtwyPort,$Password,$Name,$Value,$SybSystem))) -Credential $Credential -Verbose:$Verbose
    }
}

## Change Siebel Gateway SybSystem Paramiter ##
function Get-SBLGtwyEvtLogLevel {
    param ([ValidateSet("CommInboundRcvr","EIM","CustomAppObjMgr_enu","eCommunicationsObjMgr_enu","eProdCfgObjMgr_enu","EAIObjMgr_enu","PRMPortal_enu","WfProcMgr","WfProcBatchMgr","AsgnSrvr","XMLPReportServer","CommInboundProcessor")][string]$CompAliasName,
           [string]$GatewayServer = $global:SBLGtwyServer,
           [string]$Path          = $global:SBLGtwyPath,
           [string]$Password,
           [Management.Automation.PSCredential]$Credential,
    ...
    if ($Server) {
        $lGtwyCmd01   = '{0}\BIN\srvrmgr.exe -g {1}:{2} -e siebel -u SADMIN -p "{3}" -c "list evtloglvl for server {4} component {5}"'
        Write-Verbose -Message ("Gateway: Get Event logging level for server '{0}', compoment '{1}'." -f $Server,$CompAliasName) -Verbose:$Verbose
        Invoke-Command -ComputerName $GatewayServer -ScriptBlock ([Scriptblock]::Create(($lGtwyCmd01 -f $Path,$GatewayServer,$global:$SBLGtwyPort,$Password,$Server,$CompAliasName))) -Credential $Credential -Verbose:$Verbose
    } else {
        $lGtwyCmd01   = '{0}\BIN\srvrmgr.exe -g {1}:{2} -e siebel -u SADMIN -p "{3}" -c "list evtloglvl for component {4}"'
        Write-Verbose -Message ("Gateway: Get Event logging level for compoment '{0}'." -f $CompAliasName) -Verbose:$Verbose
        Invoke-Command -ComputerName $GatewayServer -ScriptBlock ([Scriptblock]::Create(($lGtwyCmd01 -f $Path,$GatewayServer,$global:$SBLGtwyPort,$Password,$CompAliasName))) -Credential $Credential -Verbose:$Verbose
    }
}
...
function Set-SBLGtwyEvtLogLevel {
    param ([string]$Name = "%",
           ...
           [ValidateSet("CommInboundRcvr","CommInboundRcvr","EIM","CustomAppObjMgr_enu","eCommunicationsObjMgr_enu","eProdCfgObjMgr_enu","EAIObjMgr_enu","EAIObjMgrOTP_enu", "eChannelObjMgr_enu","eChannelObjMgrAD_enu","WfProcMgr","WfProcBatchMgr","AsgnSrvr","XMLPReportServer","CommInboundProcessor")][string]$CompAliasName,
           [string]$Server,
           [string]$GatewayServer = $global:SBLGtwyServer,
           [string]$Path          = $global:SBLGtwyPath,
           [string]$Password,
...
function Set-SBLSrvrComp {
    param ([ValidateSet("CommInboundRcvr","CustomAppObjMgr_enu","eCommunicationsObjMgr_enu","eProdCfgObjMgr_enu","EAIObjMgr_enu","EAIObjMgrOTP_enu", "eChannelObjMgr_enu","eChannelObjMgrAD_enu","WfProcMgr","WfProcBatchMgr","AsgnSrvr","XMLPReportServer","CommInboundProcessor")][string]$CompAliasName,
    ...
    if ($Server) {
        $lGtwyCmd01   = '{0}\BIN\srvrmgr.exe -g {1}:{2} -e siebel -u sadmin -p "{3}" -c "{4} comp {5} for server {6}"'
        Write-Verbose -Message ("SiebelServer: {0} comp '{1}' for server '{2}'." -f $Command,$CompAliasName,$Server) -Verbose:$Verbose
        Invoke-Command -ComputerName $GatewayServer -ScriptBlock ([Scriptblock]::Create(($lGtwyCmd01 -f $Path,$GatewayServer,$global:$SBLGtwyPort,$Password,$Command,$CompAliasName,$Server))) -Credential $Credential -Verbose:$Verbose
    } else {
        $lGtwyCmd01   = '{0}\BIN\srvrmgr.exe -g {1}:{2} -e siebel -u sadmin -p "{3}" -c "{4} comp {5}"'
        Write-Verbose -Message ("SiebelServer: {0} compoment '{1}'." -f $Command,$CompAliasName) -Verbose:$Verbose
        Invoke-Command -ComputerName $GatewayServer -ScriptBlock ([Scriptblock]::Create(($lGtwyCmd01 -f $Path,$GatewayServer,$global:$SBLGtwyPort,$Password,$Command,$CompAliasName))) -Credential $Credential -Verbose:$Verbose
    }
}
...
function Get-SBLSrvrComp {
    ...
    if ($Server) {
        $lGtwyCmd01   = '{0}\BIN\srvrmgr.exe -g {1}:{2} -e siebel -u sadmin -p "{3}" -c "list comp {4} for server {5}"'
        Write-Verbose -Message ("SiebelServer: list comp '{0}' for server '{1}'." -f $CompAliasName,$Server) -Verbose:$Verbose
        Invoke-Command -ComputerName $GatewayServer -ScriptBlock ([Scriptblock]::Create(($lGtwyCmd01 -f $Path,$GatewayServer,$global:$SBLGtwyPort,$Password,$CompAliasName,$Server))) -Credential $Credential -Verbose:$Verbose
    } else {
        $lGtwyCmd01   = '{0}\BIN\srvrmgr.exe -g {1}:{2} -e siebel -u sadmin -p "{3}" -c "list comp {4}"'
...
## Siebel AI
Export-ModuleMember -Function 'Set-SBLAIAuthToken'
Export-ModuleMember -Function 'Stop-SBLAIServer'
Export-ModuleMember -Function 'Start-SBLAIServer'
Export-ModuleMember -Function 'Get-SBLAIServer'
